---
title: "Lab 9. Binary logistic regression"
---
...
fit1 <- glm(CONSTRUCTION ~ VERB, loaddata, family = binomial)
summary(fit1)
```
...
```{r 2.5}
fit3 <- glm(CONSTRUCTION ~ VERB + REDUCED + PARTICIPLE, loaddata, family = "binomial")
summary(fit3)
```
...
```{r 2.7}
fit7 <- glm(CONSTRUCTION ~ VERB + REDUCED + PARTICIPLE + VERB:PARTICIPLE, loaddata, family = "binomial")
summary(fit7)
```
...
load.glm2 <- glm(CONSTRUCTION ~ ., family=binomial, data=loaddata)
